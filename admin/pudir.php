            ...
            <div class="container-fluid">
				...
				<form action="" method="post">
					...
					if(isset($_POST['simpan'])){
						...
						$sql3=mysql_query("INSERT into akun values('$user', '$pass', 'pudir')");
						if($sql3){
							echo "<script> document.location='../admin/pudir.php'</script>";
								}else {
									echo "<script>alert('data gagal disimpan'); document.location='../admin/pudir.php'</script>";
								}
					}
					?>
					  </div>
					...
					<div class="table-responsive">
                            <table class="table table-bordered table-hover table-striped">
                                ...
                                <tbody>
									<?php
						include "../dosenp/koneksi.php";
							$sql = "SELECT * from akun where hak_akses='pudir'";
							$kueri = mysql_query($sql);
							$no=1;
					...
					?>
								...
								<td><a href="hapuspudir.php?kode=<?php echo $data['username']?>"> Hapus </a> </td>
								</tr>
							<?php
								$no++;}
							?>
...
if(isset($_POST['ubah'])){
	...
	for($i=0;$i<sizeof($pass);$i++){
		...
		if($sql2){
			...
			echo "<script>alert('data gagal di ubah'); document.location='../admin/pudir.php'</script>";
		}
	}
}
?>