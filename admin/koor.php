            ...
            <div class="container-fluid">
				...
				<form action="" method="post">
					...
					if(isset($_POST['simpan'])){
						...
						$sql3=mysql_query("INSERT into akun values('$user', '$pass', 'koordinator')");
						$sql4=mysql_query("SELECT * from akun where username='$user'");
						$data4=mysql_fetch_array($sql4);
						$sql5=mysql_query("INSERT into koordinator values('$user', '$nik', '$nama', '$prodi')");
						if($sql5){
							echo "<script> document.location='../admin/koor.php'</script>";
								}else {
									echo "<script>alert('data gagal disimpan'); document.location='../admin/koor.php'</script>";
								}
					}
					?>
					  </div>
					...
					<div class="table-responsive">
                            <table class="table table-bordered table-hover table-striped">
                                ...
                                <tbody>
									<?php
						include "../dosenp/koneksi.php";
							$sql = "SELECT * from akun where hak_akses='koordinator'";
							$kueri = mysql_query($sql);
							$no=1;
							while($data = mysql_fetch_array($kueri)){
								$username1=$data['username'];
							$sql1=mysql_query("SELECT * from koordinator where username='$username1'");
					...
					?>
								...
								<td><a href="hapuskoor.php?kode=<?php echo $data['username']?>"> Hapus </a> </td>
								</tr>
							<?php
								$no++;}
							?>
...
if(isset($_POST['ubah'])){
	...
	for($i=0;$i<sizeof($pass);$i++){
		...
		if($sql2){
			...
			echo "<script>alert('data gagal di ubah'); document.location='../admin/koor.php'</script>";
		}
	}
}
?>